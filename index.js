...
const { getToken, generic_lists, list_catalog, list_cat, listOfLists, getMeta, search, getUserProfile, upNext } = require('./trakt-api.js');
...
app.get('/:configuration?/configure', cacheHeaders, (req, res) => {
    ...
    config.up_next = req.query.up_next === 'true';
...
app.get('/:configuration?/manifest.json', async (req, res) => {
    try {
        ...
        if (config.up_next) {
            const upNextData = await upNext(config.access_token);
            newManifest.up_next = upNextData;
        }
